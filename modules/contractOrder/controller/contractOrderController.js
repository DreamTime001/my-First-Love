(function () {
    ...
    angular.module('app.controllers')
        .controller('contractOrderController', ['$api', '$timeout', '$upload', '$http', '$state', '$cookies', '$location', '$anchorScroll', '$layer', '$util', function ($api, $timeout, $upload, $http, $state, $cookies, $location, $anchorScroll, $layer, $util) {
            var vm = this;
            $timeout(function () {
                initMenu();
            }, 100);
            ...
            vm.getPagedDataAsync = function () {
                vm.orgCd = '';
                $api.post('order/selectOfferAsset', {query: angular.toJson(vm.query)}, function (result) {
                    vm.list = result.data.list;
                    for (var i = 0; i < vm.list.length; i++) {
                        vm.list[i].checked = false;//添加默认选中状态
                        vm.list[i].crtTm = vm.list[i].crtTm.replace(/(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})/g, '$1-$2-$3 $4:$5:$6');//日期转换
                    }
...
//-------------------------------------------------------------------------------分割线------------------------------------------------------------------------------------//
            ...
            vm.selectThis = function (obj) {
                ...
                obj.checked = true;
                vm.sts = obj.sts;
                vm.stcd = obj.stoffCd;//资产编号
                vm.orgCd = obj.org_cd;//机构编号
                vm.ordrCd = obj.ordr_cd;//订单编号
                vm.orgcdm = obj.org_cd_m;//受让方机构编号
                vm.crt_p_id = obj.id_flagB;//受让方 1-个人 2-企业
                vm.id_flag = obj.id_flagS;//出让方 1-个人 2-企业
            };
            //重置
            ...
            vm.assetView = function () {
                if (vm.orgCd) {
                    $api.post('asset/getAsset', {st_cd: vm.stcd, ty: 9}, function (result) {
                        vm.titleView = '资产信息';
...
//---------------------------------------------------------------------------------------------弹出层--------------------------------------------------------------------------//
            ...
            vm.viewTransInfo = function () {
                if (vm.orgCd) {
                    $api.post('order/showOfferAsset', {stcd: vm.stcd}, function (result) {
                        vm.titleView = '交易信息';
